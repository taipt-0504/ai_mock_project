...
| Global gate proxy | `proxy.ts` at the project root | Reads `config.SAA_LAUNCH_AT` per request, redirects non-whitelisted routes to the prelaunch path while the gate is active. See FR-006 / FR-007. **No `proxy.ts` exists today** — this is greenfield infrastructure. **Next.js 16 convention note**: the project is on Next.js 16.2.4 (Constitution § Technology Stack), where `proxy.ts` is the request-interception convention (renamed from `middleware.ts` in Next.js 16; see [upgrade guide](https://nextjs.org/docs/app/guides/upgrading/version-16#middleware-to-proxy)). The exported function name is `proxy`, not `middleware`. The `proxy` runtime is Node.js — Edge runtime is not supported (TR-001). |
...
- The proxy is **new infrastructure**: no `proxy.ts` exists today. Plan-phase MUST scope the matcher config (FR-007 whitelist) and the test surface (TR-004) accordingly.
