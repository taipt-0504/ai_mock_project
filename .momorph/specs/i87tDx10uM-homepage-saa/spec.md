...
3. **Given** the configured event start datetime is now in the past, **When** the screen renders, **Then** the entire countdown block (tiles + "Coming soon" subtitle) is hidden, no further ticking is attempted, and the page continues to render the rest of the hero (key visual + `EventInfo` + CTAs). _(Revised 2026-05-10: previously the spec required `00 / 00 / 00` tiles + only the subtitle hidden — but a "ghost" countdown showing zeros suggested the event was still pending, so the whole block now unmounts.)_
...
5. **Given** the page has been open for more than a minute, **When** the user looks at the event metadata block (`B2`), **Then** they see `Thời gian: <SAA_EVENT_START_AT formatted as DD/MM/YYYY in vi-VN, "Month D, YYYY" in en-US, ICT timezone>`, `Địa điểm: <localized venue name>`, plus the supporting `Tường thuật trực tiếp qua sóng Livestream` note. The location and Facebook copy come from the i18n catalog; the time value derives from the same `SAA_EVENT_START_AT` env that drives the countdown so a single source of truth governs both. _(Revised 2026-05-10: previously the time value was a hardcoded i18n string; now derives from env to keep the page in sync when the event date moves.)_
...
| `B1` Countdown section          | `2167:9035`                            | Wraps `B1.2` subtitle and `B1.3` tile group.                                                                      | Auto-updates per minute against `SAA_EVENT_START_AT`. Entire block hides once the target is reached (FR-008 revised 2026-05-10). |
| `B1.2` "Coming soon" subtitle   | `2167:9036`                            | Label only.                                                                                                       | Hidden when `now >= eventStart` — bundled with the rest of `B1` (whole section unmounts).                                 |
| `B1.3` Countdown tile group     | `2167:9037`                            | Wraps the three tiles below.                                                                                      | Container; per-tile behavior delegated.                                                                                   |
| `B1.3.1` Days tile              | `2167:9038`                            | 2-digit zero-padded numeric tile + `DAYS` label. **Each digit renders inside its own glassmorphic LED tile** (Figma "Digital Numbers" / 7-segment display, two tiles per unit) — see Phase 14 visual redesign in `tasks.md`. | Auto-updates per minute. Renders `00 00` once event start is in the past.                                                |
| `B1.3.2` Hours tile             | `2167:9043`                            | 2-digit zero-padded + `HOURS` label. Same two-LED-tile structure as `B1.3.1`.                                     | Same behavior as `B1.3.1`.                                                                                                |
| `B1.3.3` Minutes tile           | `2167:9048`                            | 2-digit zero-padded + `MINUTES` label. Same two-LED-tile structure as `B1.3.1`.                                   | Same behavior as `B1.3.1`.                                                                                                |
...
- **FR-008**: When `now >= SAA_EVENT_START_AT`, the countdown component MUST hide its entire output (tiles + subtitle) and stop ticking. Re-render on the next mount picks this up automatically. _(Revised 2026-05-10: replaces the prior `00 / 00 / 00 + hide subtitle` rule, which left a misleading "ghost" zero-state on the page.)_
- **FR-009**: Event info block (`B2`) MUST render `Thời gian: <date>`, `Địa điểm: <localized venue>`, plus the supporting Facebook live note. The **time value MUST derive from `SAA_EVENT_START_AT`** (formatted `DD/MM/YYYY` for vi-VN, `Month D, YYYY` for en-US, both pinned to ICT) — no separate hardcoded i18n value. The location label/value and Facebook note continue to come from the i18n catalog. When `SAA_EVENT_START_AT` is unset / unparseable, the time value falls back to the i18n string `home.event.time.value` so the row never renders empty. _(Revised 2026-05-10: previously all three strings were hardcoded i18n; the time portion now tracks the same env that drives the countdown so a single source of truth governs both.)_
