...
- [ ] T001 [P] [UI] Download six 336×336 award thumbnails from Figma frame `zFYDgyj_pD` via `get_media_files` / `list_media_nodes` against design items `D.1.1_Picture-Award` (and equivalents in `D.2`–`D.6`); save with slug-based filenames | public/assets/awards/{top-talent,top-project,top-project-leader,best-manager,signature-2025-creator,mvp}.png
- [ ] T002 [P] [UI] Run `query_section` on Figma nodes `A_Title`, `B_Hệ thống giải thưởng`, `C_Menu list`, `C.1`–`C.6`, `D.1`–`D.6`, `D2.1` to extract text colours, surface colours, active-menu colour, card meta-text colour. Add the four new tokens (`--color-saa-awards-nav-active`, `--color-saa-awards-nav-fg`, `--color-saa-awards-card-meta-fg`, `--saa-header-scroll-margin`) to `:root` in `app/globals.css`. Confirm `--saa-header-scroll-margin = 0px` (Header is `position: absolute` per [src/components/header/Header.tsx](../../src/components/header/Header.tsx) — no occlusion to compensate for) | app/globals.css
- [ ] T003 [P] [UI] Re-survey the Awards-page Header node in Figma to confirm it is NOT sticky/fixed (matches the deployed absolute-position pattern). If Figma indicates sticky, file a separate Header-component change ticket and STOP this phase — header reuse rule per FR-009 is out-of-scope for this plan
- [ ] T004 [Logic] Add the 9 new `awards.detail.*` keys to `vi-VN.json` per the i18n table in plan.md §"Integration Points → i18n integration": `title_caption`, `title_heading`, `quantity_label`, `value_label`, `unit.don_vi`, `unit.tap_the`, `unit.ca_nhan`, `value_individual`, `value_team` | src/lib/i18n/catalogs/vi-VN.json
- [ ] T005 [Logic] Add the same 9 `awards.detail.*` keys to `en-US.json` with the **draft English copy** from plan.md (confirmed with PO before commit). Run [tests/unit/lib/i18n/parity.test.ts](../../tests/unit/lib/i18n/parity.test.ts) — must stay green after both catalogs are updated | src/lib/i18n/catalogs/en-US.json
- [ ] T006 [Test] Re-run the existing i18n parity test to confirm the new keys exist in both catalogs and no extra keys were introduced | (read-only) tests/unit/lib/i18n/parity.test.ts
