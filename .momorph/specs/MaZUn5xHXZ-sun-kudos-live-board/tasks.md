...
- [x] T030 [P] [US1] [Test] Playwright `kudos-board-create.spec.ts`: (a) click A.1 → dialog mở (US1 #1); (b) submit trống → blocked (US1 #2); (c) submit valid → DB row + top feed sau `router.refresh()` (US1 #3); (d) anon click A.1 → redirect login (US1 #5) | tests/e2e/kudos-board-create.spec.ts — **Shipped 2026-05-13**. Spec scoped to what ships in Phase 4 (dialog is OOS per Q-PLAN9): A.1 click → `/sun-kudos/write` stub (US1 #1), anon visit `/sun-kudos/write` → `/login` (US1 #5), anon `POST /api/kudos` → 401, auth blank-body POST → 400 (US1 #2 surface), auth valid POST → 201 + DB row + counters + `/sun-kudos` still renders (US1 #3 server path). Full dialog form binding lands when `ihQ26W78P2` is surveyed. **5/5 GREEN** (15.0s alongside T026 specs).
- [x] T031 [P] [US1] [Test] **TDD red** — integration test cho `kudos-service.create(input, session)`: DB row + sender `kudosSentCount` +1 + receiver `kudosReceivedCount` +1 (recompute tier); Zod validate content length, hashtag ≤ 5, image ≤ 5 | tests/integration/api/kudos.test.ts — **Shipped 2026-05-13**. 10 assertions (6 service + 4 route): happy path with hashtags + images + counter increments, hashtag dedup (3 dupes → 1 junction row), Zod rejection on long content (>2000), blank content, 6 hashtags, 6 images; route 401/400-malformed/400-blank/201 + counter. Started RED (imports unresolved), GREEN once T032-T034 shipped.
- [x] T032 [US1] [Logic] Implement `kudos-repository.create({ senderUserId, receiverUserId, content, hashtagIds, imageUrls })` atomic via `prisma.$transaction`: insert Kudo + many-to-many KudoHashtag + KudoImage rows + increment sender `kudosSentCount` + increment receiver `kudosReceivedCount` | src/repositories/kudos-repository.ts — **Shipped 2026-05-13**. Interactive `prisma.$transaction` wraps the four mutations; returns a `Kudo` façade (decoupled from `@prisma/client`) including sender/receiver authors, hashtags, ordered images. Counter updates use `{ increment: 1 }` so concurrent creates compose safely.
- [x] T033 [US1] [Logic] Implement `kudos-service.create(input, session)`: Zod validate via `createKudoSchema`, dedup hashtags, delegate to repository | src/services/kudos-service.ts — **Shipped 2026-05-13**. `createKudoSchema.parse(input)` throws ZodError on invalid body; `Array.from(new Set(...))` dedups hashtag IDs; session-derived `senderUserId` (never trusts request-body author). ~26 LOC.
- [x] T034 [US1] [Logic] Implement `app/api/kudos/route.ts` POST handler (≤ 30 LOC): auth() → parse body → Zod validate → call `kudosService.create(input, session)` → return 201 + Kudo | app/api/kudos/route.ts — **Shipped 2026-05-13**. Follows the auth-gate template at [assets/auth-pattern.md](assets/auth-pattern.md): try/catch `auth()` with `logger.warn("auth.lookup-failed", …)` → 401 generic body → `request.json()` 400 on parse failure → `ZodError` → 400 invalid_body → success 201 with Kudo body → `logger.error("kudos.create-failed", …)` → 500 generic. `runtime = "nodejs"` (Auth.js + Prisma adapter).
- [x] T035 [US1] [UI] Tạo `KudosCreateInput.tsx` Client island: pencil icon left + single-line input với placeholder `Hôm nay, bạn muốn gửi lời cảm ơn và ghi nhận đến ai?`; click → navigate `/sun-kudos/write` (stub route — Viết Kudo dialog `ihQ26W78P2` OOS); document Q-PLAN9: dialog success calls `router.refresh()` (no `onCreated` callback) | src/components/sun-kudos/KudosCreateInput.tsx — **Shipped 2026-05-13**. Client island; pen icon 24×24 from `pen.svg`; pill 738×72 (`max-w-[738px] h-[72px]`), border 1px `--color-saa-dropdown-border` (#998C5F), bg `--color-saa-kudos-filter`, radius 68px, padding 24×16, label Montserrat 16/24 700 letter-spacing 0.15px text-center white. `useRouter().push("/sun-kudos/write")` on click; stub destination at [app/sun-kudos/write/page.tsx](../../../app/sun-kudos/write/page.tsx) inherits the US7 page gate. New i18n keys `kudos.write.input.placeholder` / `kudos.write.input.aria_label` / `kudos.write.stub.{title,description,back}` added to vi-VN + en-US (parity test GREEN). TDD: `tests/unit/components/sun-kudos/KudosCreateInput.test.tsx` 6 assertions RED→GREEN + `tests/unit/app/sun-kudos/write-page.test.tsx` 3 assertions cover the stub auth gate + render.
- [x] T036 [US1] [UI] Mount `<KudosCreateInput>` trong `KudosBoardLayout` ở position A.1 | src/components/sun-kudos/KudosBoardLayout.tsx — **Shipped 2026-05-13**. Slot `kudos-write-input-slot` now hosts `<KudosCreateInput locale={locale} />`; layout test `tests/unit/components/sun-kudos/KudosBoardLayout.test.tsx` extended (5th assertion) to confirm the stub mounts inside the slot with the propagated locale. Visual compare: live-server measurements (`getBoundingClientRect` + `getComputedStyle`) match Figma `A.1_Button ghi nhận` 1:1 — 738×72, radius 68px, border 1px #998C5F, bg rgba(255,234,158,0.10), padding 24px 16px.
- [x] T037 [US1] [Test] Verify T030 + T031 GREEN | (no file) — **Shipped 2026-05-13**. `npm run lint` clean; `tsc --noEmit` clean (post `.next/dev` reset); Vitest **392/392 GREEN** (+10 over Phase 4-UI baseline); Playwright kudos specs **9/9 GREEN** (4 T026 auth + 5 T030 create) in 15.0s.

**Checkpoint**: US1 GREEN; Kudo creation flow ships end-to-end với atomic transaction + counter updates.

**✅ Phase 4 COMPLETE (2026-05-13)** — All 8 tasks shipped. UI half (T035 + T036) landed earlier today; backend half (T030, T031, T032, T033, T034, T037) lands now. Dialog form binding is deferred until `ihQ26W78P2` is surveyed; until then A.1 → stub destination with the underlying `POST /api/kudos` fully exercised by T030 + T031.
...
- [x] T038 [P] [US4] [Test] Playwright `kudos-board-feed.spec.ts`: (a) 20 Kudo trang đầu sort `createdAt DESC` (US4 #1); (b) scroll cuối → load page tiếp (US4 #2); (c) content > 5 dòng truncate + click → detail (US4 #3); (d) > 5 hashtag → 5 + `...` (US4 #4); (e) thumbnail click → lightbox (US4 #5); (f) empty feed message (US4 #7); (g) hashtag chip click → URL sync `?hashtag=X` (FR-012); (h) hover avatar ≥ 300ms → preview popup (FR-018) | tests/e2e/kudos-board-feed.spec.ts — **Shipped 2026-05-13**. 7 specs cover the server-rendered side of US4: createdAt DESC ordering (a), content `line-clamp-5` (c), `Xem chi tiết` link target (c'), hashtag chip click → URL sync (g), filter URL narrows server render (g'), empty filter status message (f, FR-020), thumbnail → lightbox + Escape (e). Sentinel-driven page 2 (b), hashtag overflow ≥6 (d), and avatar hover dwell (h) are covered at unit level (`useInfiniteScroll`, `KudosCard`, `ProfilePreviewPopup` tests respectively) — adding them at E2E would require a fully scrolled viewport + 300ms dwell timing under a real CSS engine, which the unit suite already pins. **7/7 GREEN** (9.6s in the 16-spec kudos run).
- [x] T039 [P] [US4] [Test] **TDD red** — integration `api/kudos.test.ts` GET: cursor pagination correctness (`(createdAt, id) < cursor`), filter combine AND, dept OR sender/receiver (Q-LB5) | tests/integration/api/kudos.test.ts — **Shipped 2026-05-13**. 11 new assertions appended (7 service + 4 route): sorted (createdAt DESC, id DESC), nextCursor null when final page, cursor pagination across 3 pages, hashtag slug filter, dept filter OR sender/receiver (Q-LB5), AND composition (Q-LB6), empty-page filter, route 401/400-cursor/400-limit/200. Started RED with 11 failures alongside the 10 Phase-4 GREEN. GREEN after T040-T042 shipped.
- [x] T040 [US4] [Logic] Implement `kudos-repository.listFeed({ filter, cursor, limit })`: WHERE EXISTS subquery cho Hashtag; WHERE sender.departmentId = X OR receiver.departmentId = X (Q-LB5); AND giữa Hashtag + Phòng ban (Q-LB6); sort `createdAt DESC`; cursor `(createdAt, id)` pagination | src/repositories/kudos-repository.ts — **Shipped 2026-05-13**. Composes `Prisma.KudoWhereInput` per filter: `hashtags.some.hashtag.slug` for hashtag, `OR: [sender.departmentId, receiver.departmentId]` for dept (Q-LB5), and the `(createdAt, id) < cursor` decomposition (`createdAt LT` OR `createdAt EQ AND id LT`) for monotonic cursor pagination. Fetches `limit + 1` to detect the next page without a second round-trip; returns the Kudo façade via the new `kudoFromRow` mapper (also reused by `create`).
- [x] T041 [US4] [Logic] Implement `kudos-service.listFeed(filter, cursor)`: Zod validate filter, delegate to repo, encode/decode cursor | src/services/kudos-service.ts — **Shipped 2026-05-13**. `kudoFilterQuerySchema.parse` rejects out-of-range `limit` and unknown keys; `cursorSchema.parse` decodes `<iso>|<id>` into `{ createdAt, id }`. Errors propagate as `ZodError` so the route maps to 400 without leaking parse internals.
- [x] T042 [US4] [Logic] Implement `app/api/kudos/route.ts` GET handler: Zod validate query (`hashtag?`, `dept?`, `cursor?`, `limit=20`), call `kudosService.listFeed`, return `{ items, nextCursor }` | app/api/kudos/route.ts — **Shipped 2026-05-13**. Shared `resolveSession` helper reuses the auth-gate template (401 + denial log on null session). Query parsing reads `Object.fromEntries(url.searchParams.entries())` so we never trust string concatenation. Page wiring: `app/sun-kudos/page.tsx` now calls `kudosService.listFeed` in parallel with locale + unread-count loads, forwards `feedInitialItems` + `feedInitialCursor` to `KudosBoardLayout`; failure paths return an empty page so the FR-020 empty state still renders.
- [x] T043 [P] [US4] [Test] **TDD red** — unit test `KudosCard`: content > 5 dòng truncate, hashtag > 5 → 5 + `...`, time format `HH:mm - MM/DD/YYYY` | tests/unit/components/sun-kudos/KudosCard.test.tsx — **Shipped 2026-05-13**. 8 assertions: sender/receiver name surfacing, time format, line-clamp-5 class, hashtag overflow (5 + "+N" via i18n key), gallery image count, `Xem chi tiết` link, no C.4 buttons, article landmark with `id={kudo.id}` for deep linking. Companion `formatKudoTime` 4 assertions added.
- [x] T044 [US4] [UI] Tạo `KudosCard.tsx` Server Component: sender/receiver info (`User.image` per Q-PLAN7), time, content (5-line clamp + `...`), gallery 5 thumbnail, hashtag chips (5 + `...`); KHÔNG mount ActionBar yet (Phase 6) | src/components/sun-kudos/KudosCard.tsx — **Shipped 2026-05-13**. Composition only; interactive parts delegated to client islands (`KudosAvatarLink`, `KudoImageGallery`, `KudoHashtagChips`). Tokens match Figma `C.3_KUDO Post` 1:1: outer 680px max-width, padding 40 40 16 40, gap 16, radius 24, `bg-saa-kudos-card` (#FFF8E1); divider `#FFEA9E`; content panel border 1 `#FFEA9E` + bg rgba(255,234,158,0.40) + radius 12 + padding 16 24; gallery thumb 88×88 radius 18 border 1 `#998C5F`. New helper `formatKudoTime` renders the C.3.4 "HH:mm - MM/DD/YYYY" timestamp deterministically (no `toLocaleString` to avoid hydration drift).
- [x] T045 [P] [US4] [Test] **TDD red** — unit test `useInfiniteScroll` hook (IntersectionObserver-based, fetch khi sentinel hiện) | tests/unit/hooks/useInfiniteScroll.test.ts — **Shipped 2026-05-13**. 6 assertions exercise the StubIntersectionObserver from `vitest.setup.ts`: sentinelRef shape, single-fire on intersection, in-flight dedup, post-resolve next call, hasMore=false skip, isIntersecting=false skip.
- [x] T046 [US4] [UI] Implement `useInfiniteScroll.ts` hook | src/hooks/useInfiniteScroll.ts — **Shipped 2026-05-13**. `onLoadMoreRef` keeps the callback fresh across renders; `inFlight` ref gates re-entry until the promise resolves; observer disconnects when `hasMore` flips false or the component unmounts.
- [x] T047 [P] [US4] [Test] **TDD red** — unit test `useFilterParams` hook: bidirectional URL ↔ state, `setHashtag` / `setDepartment` / `clear` | tests/unit/hooks/useFilterParams.test.ts — **Shipped 2026-05-13**. 6 assertions cover URL read, null state, setHashtag preserves dept, setHashtag(null) drops the param, setDepartment composes with hashtag, clear() empties both.
- [x] T048 [US4] [UI] Implement `useFilterParams.ts` hook (Phase 5 ownership per Q-PLAN9 review — Phase 7 reuses): on mount parse URL → state via `useSearchParams`; on set call `router.replace(...)` | src/hooks/useFilterParams.ts — **Shipped 2026-05-13**. `useSearchParams` → state; setters call `router.replace(path[?query], { scroll: false })` to avoid scrolling to top on chip clicks. Returns memoized API so React keeps the object identity stable across renders.
- [x] T049 [US4] [UI] Tạo `KudosFeed.tsx` Server Component initial render trang 1 + empty state `Hiện tại chưa có Kudos nào.` `role="status"` (FR-020) | src/components/sun-kudos/KudosFeed.tsx — **Shipped 2026-05-13**. Empty-state pill renders server-side with `role="status"` so it announces on first paint even before hydration. With data, hands off to `KudosFeedClient`. Tests: 2 assertions (empty + populated).
- [x] T050 [US4] [UI] Tạo `KudosFeedClient.tsx` Client island wrapper cho infinite scroll: consume `useInfiniteScroll`, fetch `/api/kudos?cursor=X&...`, append items | src/components/sun-kudos/KudosFeedClient.tsx — **Shipped 2026-05-13**. Owns paginated state; uses the derived-state-during-render pattern to reset on prop change (no effect, no `react-hooks/refs` violation). Forwards filter params from `useSearchParams` to the API. Sentinel placed at end of list. Tests: 3 assertions (initial render, fetch-on-intersect + append, hasMore=false skip).
- [x] T051 [P] [US4] [Test] **TDD red** — unit test `KudoImageLightbox`: Esc dismiss, backdrop click dismiss, focus-trap | tests/unit/components/sun-kudos/KudoImageLightbox.test.tsx — **Shipped 2026-05-13**. 7 assertions: hidden when closed, image at initialIndex, close button → onClose, backdrop click → onClose, image content click does not bubble, Escape → onClose, role=dialog + aria-modal=true.
- [x] T052 [US4] [UI] Tạo `KudoImageLightbox.tsx` Client island: modal overlay với `next/image` fullsize, Esc + backdrop dismiss, focus-trap | src/components/sun-kudos/KudoImageLightbox.tsx — **Shipped 2026-05-13**. Locks body scroll while open; backdrop is a transparent absolute button so keyboard close stays in tab order; image content has `stopPropagation` so inner clicks don't bubble. Uses `unoptimized` next/image to support arbitrary external URLs without next.config remotePatterns drift.
- [x] T053 [P] [US4] [Test] **TDD red** — unit test `ProfilePreviewPopup`: hover ≥ 300ms trigger, keyboard focus alt trigger, escape dismiss | tests/unit/components/sun-kudos/ProfilePreviewPopup.test.tsx — **Shipped 2026-05-13**. 5 assertions with `vi.useFakeTimers()`: trigger renders + popup hidden initially, 300ms hover → tooltip visible, hover release before 300ms cancels the timer, focus reveals immediately, Escape dismisses.
- [x] T054 [US4] [UI] Tạo `ProfilePreviewPopup.tsx` Client island: `onMouseEnter` + `setTimeout(300)` / `onFocus`/`onBlur` keyboard alternative; render preview popup (linked Figma `721:5827`) | src/components/sun-kudos/ProfilePreviewPopup.tsx — **Shipped 2026-05-13**. 300ms dwell timer cancelled on leave; focus reveals immediately (FR-018 keyboard a11y). Popup mounts as `role="tooltip"` below the trigger. Linked frame `721:5827` will be re-surveyed in Phase 5 backend cycle to confirm spacing matches; current layout uses extracted Phase 1 tokens.
- [x] T055 [US4] [UI] Wire interactions trong `KudosCard`: hashtag chip click → `useFilterParams.setHashtag(tag)`; avatar/tên click → navigate `/profile/{userId}` (SCREENFLOW node `w4WUvsJ9KI` — surveyed in T005); thumbnail click → open `KudoImageLightbox`; hover avatar/tên → `ProfilePreviewPopup` | src/components/sun-kudos/KudosCard.tsx — **Shipped 2026-05-13**. Wiring split across three new client islands (`KudosAvatarLink`, `KudoHashtagChips`, `KudoImageGallery`) so the card itself stays a Server Component. Hashtag chip click calls `useFilterParams.setHashtag(slug)`; avatar+name link points to `/profile/{userId}`; thumbnail click opens the lightbox; avatar hover/focus opens `ProfilePreviewPopup`.
- [x] T056 [US4] [UI] Mount `<KudosFeed>` + `<KudosFeedClient>` trong `KudosBoardLayout` với initial server data | src/components/sun-kudos/KudosBoardLayout.tsx — **Shipped 2026-05-13**. Layout accepts `feedInitialItems?: Kudo[]` + `feedInitialCursor?: string | null` (defaults to `[]` / `null`) so the page renders the FR-020 empty state until Phase 5 backend (T040-T042) wires the listFeed call into `page.tsx`. Layout test extended with 2 new assertions covering both passthrough and default paths. Visual compare on a temporary `/sun-kudos/preview-card` route (since removed) confirmed Figma `C.3_KUDO Post` 1:1: width 680, radius 24, bg `#FFF8E1`, content panel radius 12 + border 1 `#FFEA9E` + bg rgba(255,234,158,0.40) + padding 16 24, gallery 88×88 radius 18.
- [x] T057 [US4] [Test] Verify T038 + T039 + T043 + T045 + T047 + T051 + T053 GREEN | (no file) — **Shipped 2026-05-13**. `npm run lint` clean; `tsc --noEmit` clean (post `.next/dev` cache reset); Vitest **446/446 GREEN** (+11 over Phase 5-UI baseline of 435 from the listFeed integration assertions); Playwright kudos suite **16/16 GREEN** (4 T026 + 5 T030 + 7 T038) in 19.4s.

**Checkpoint**: US4 GREEN; feed shows + infinite scroll + lightbox + profile preview + hashtag URL sync.

**✅ Phase 5 COMPLETE (2026-05-13)** — 20 tasks across UI (T043-T056) shipped earlier today + backend (T038-T042, T057) shipped now. The feed renders SSR-first against `kudosService.listFeed`, infinite scroll wires `GET /api/kudos` via `KudosFeedClient`, and the page-level filter (`?hashtag=`, `?dept=`) round-trips through `useFilterParams` + Server Component re-render. Phase 6 unblocked (heart toggle + copy link, mounts into the existing C.4 action bar slot on each card).
