...
| Route trang Hồ sơ                         | `/profile` (chưa hiện thực — link đã render, chấp nhận 404 ngắn hạn theo Resolved Question Q1)                  |
...
3. **Given** route `/profile` chưa được hiện thực (theo Resolved Question Q1, chấp nhận 404 ngắn hạn), **When** người dùng click "Hồ sơ", **Then** trình duyệt vẫn điều hướng tới URL đó; trang đích trả về 404 — đây là vấn đề của trang Profile, KHÔNG phải lỗi của overlay.
...
- **Ra**:
  - `A.1` Hồ sơ → `/profile` (route đích chưa hiện thực; chấp nhận 404 ngắn hạn theo Resolved Question Q1).
...
- [ ] Trang `/profile` (destination) — pending; theo Q1 (resolved), chấp nhận 404 ngắn hạn cho tới khi spec Profile ra
...
- **Q1 — Phạm vi route `/profile` ở milestone hiện tại.** **Resolved 2026-05-08: chấp nhận 404 ngắn hạn (option a).** Mục "Hồ sơ" giữ nguyên link tới `/profile`; nếu user click khi route chưa có, trình duyệt rơi vào 404 — chấp nhận được vì component-level đúng và 404 là vấn đề của trang đích. Tài liệu hoá ở US2 scenario 3. Task survey/spec cho `/profile` đặt vào backlog của SCREENFLOW Next Steps (đã thêm 2026-05-08).
