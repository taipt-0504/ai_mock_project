...
- **Created**: 2026-05-07
- **Last Updated**: 2026-05-10 (Prelaunch Phase 16 — demo gate-bypass cookie. Reviewers can click "Skip pre-launch (demo)" on `/coming-soon` to set an httpOnly `saa_gate_bypass=1` cookie (7-day TTL); the proxy honors it as "gate lifted for THIS user only". Banner on post-gate pages clears it. README highlights the demo intent. Vitest 343/343, Playwright `gate-active` 20/20 + `gate-disabled` 54/54 GREEN.)
...
flowchart TD
    ...
    subgraph Main["Main Application (post-gate)"]
        ...
        Awards["Hệ thống giải /<br/>Awards Information<br/>zFYDgyj_pD<br/>shipped 2026-05-10"]
...
| Hệ thống giải / Awards Information (`zFYDgyj_pD`, route `/awards`) | **Shipped 2026-05-10.** Read-only catalog of all six SAA 2025 award categories with description, quantity, and monetary value. Two-column desktop layout: scroll-tracking `AwardsNav` left (active card flips via IntersectionObserver) + vertical `D.1`–`D.6` `AwardDetailCard`s right. Hidden below `lg:` per Q-HTG4 — narrow viewports rely on native scroll only. Reuses `Header`, `Footer`, `KudosBlock`. Deep-links from Homepage award cards (`#<award-slug>`) anchor to the matching `D.*` card; unknown slugs fall back to `top-talent`. Implementation at [app/awards/page.tsx](app/awards/page.tsx) + [src/components/awards/](src/components/awards/); plan/tasks at [specs/zFYDgyj_pD-he-thong-giai/](specs/zFYDgyj_pD-he-thong-giai/). | Header `A1.3` `Awards Information`, footer `7.3` link, `B3.1` `ABOUT AWARDS` CTA, any award card `C2.1`–`C2.6` on Homepage with `#<award-slug>` |
...
|------|--------|----------------------|-------|
| 2026-05-10 | Demo affordance (Prelaunch Phase 16) | Prelaunch — `proxy.ts` + `prelaunch-gate.ts` + new `gate-bypass` cookie + `GateBypassPrompt` + `GateBypassBanner` + i18n + README | **Demo gate-bypass cookie.** Reviewers asked for a way to flip past the pre-launch gate during a live demo without rebuilding the deploy or editing env files. Solution: a single button below the countdown calls a Server Action that sets an httpOnly cookie `saa_gate_bypass=1` (path `/`, sameSite `lax`, 7-day TTL) and redirects to `/`; the proxy reads the cookie via `hasGateBypassCookie(request)` and forwards a 4th `bypassActive` arg to `evaluateGate(...)`. When true, the gate behaves as if `launchAt` were already in the past for THAT request only — including overriding the FR-009 fail-closed null branch (the bypass is explicit user consent). A small banner mounted in `app/layout.tsx` surfaces only when the cookie is present, with a "Turn off bypass" form action that clears the cookie and redirects to `/coming-soon`. The FR-003 / US4 invariant ("no buttons / no `<a>`") tightens to "no `<a>`, exactly one `<button>` — the demo bypass" (see new FR-010a in [Prelaunch spec.md](specs/8PJQswPZmU-countdown-prelaunch-page/spec.md)). README carries a `> For reviewers` callout pointing to [src/lib/cookies/gate-bypass.ts](src/lib/cookies/gate-bypass.ts) where a removal annotation is documented; in a real production deploy the bypass would be removed before launch. TDD red→green: 23 unit/integration assertions + 2 dedicated E2E + 4 invariant updates. Branch `feat/demo-gate-bypass` (off `main`). Vitest 343/343 + Playwright `gate-active` 20/20 + `gate-disabled` 54/54 GREEN. New phase tracked as Phase 16 / T044–T048 in [specs/8PJQswPZmU-countdown-prelaunch-page/tasks.md](specs/8PJQswPZmU-countdown-prelaunch-page/tasks.md). |
| 2026-05-10 | Behavior fix (Phase 15) | Homepage SAA — `Countdown` + `EventInfo` + `Hero` + i18n catalogs | **Countdown zero-state + env-driven event date.** Two follow-ups raised after live use of `/`: (1) when the countdown reaches zero the whole block now unmounts (FR-008 revised) — the previous `00 / 00 / 00 + hidden subtitle` left a "ghost" countdown that suggested the event was still pending; (2) the `EventInfo` time row now derives from `SAA_EVENT_START_AT` via a new pure helper `formatEventDate(date, locale)` pinned to `Asia/Ho_Chi_Minh` (FR-009 revised) — vi-VN renders `DD/MM/YYYY`, en-US renders `Month D, YYYY`, and a missing/malformed env falls back to the i18n catalog's TBD string (`Sắp công bố` / `To be announced`). Implemented TDD red→green per Constitution V; six new/rewritten unit assertions plus the existing scenario 4 fallback path. Vitest 46 files / 323 tests GREEN; targeted Playwright suites (`auth-redirect`, `language-switch`, `notification-bell`) GREEN. New phase tracked as Phase 15 / T079-T081 in [specs/i87tDx10uM-homepage-saa/tasks.md](specs/i87tDx10uM-homepage-saa/tasks.md). |
| 2026-05-10 | Bug fix (Phase 7) | Hệ thống giải — `AwardsLayout` + `app/awards/page.tsx` | **Sticky left-menu fix.** Code review caught that the spec/plan repeatedly call the left menu "sticky", but Phase 3's layout used only `lg:items-start` — the menu top-aligned within the flex row but didn't pin during scroll. Two-layer fix: (1) wrapper changed to `lg:sticky lg:top-[var(--saa-header-scroll-margin)] lg:block` reusing the existing CSS-var hook (so a future sticky/fixed Header conversion picks it up automatically); (2) `<main>` swapped from `overflow-hidden` to `overflow-x-clip` because `overflow: hidden` establishes a scroll container that breaks `position: sticky` on descendants, while `overflow: clip` clips visually without that side-effect. TDD red→green: a failing test was added first (menu stays in viewport after `scrollIntoViewIfNeeded("article#top-project-leader")`), then the fix landed. Awards e2e 20/20 GREEN. New task tracked as Phase 7 / T040 in [tasks.md](specs/zFYDgyj_pD-he-thong-giai/tasks.md). |
| 2026-05-10 | UI implementation | Hệ thống giải / Awards Information (`zFYDgyj_pD`) | Phases 1–6 of [specs/zFYDgyj_pD-he-thong-giai/tasks.md](specs/zFYDgyj_pD-he-thong-giai/tasks.md) shipped. **Phase 1 (Setup):** added `--saa-header-scroll-margin` to `app/globals.css` and 9 new `awards.detail.*` keys in lockstep across vi-VN / en-US (parity test green); confirmed reused Homepage `labelAsset` + `award-bg.png` so no new asset download needed. **Phase 2 (Foundation):** extended the static `Award` type at [src/lib/awards/awards.ts](src/lib/awards/awards.ts) with `quantity` / `unitKey` / `valueVND` / `valueVNDSecondary`; added pure helper [src/lib/awards/format.ts](src/lib/awards/format.ts) (`formatVndAmount(n).toLocaleString("vi-VN")`). **Phase 3 (US1 + US5):** [src/components/awards/AwardDetailCard.tsx](src/components/awards/AwardDetailCard.tsx), [AwardsList.tsx](src/components/awards/AwardsList.tsx), [AwardsLayout.tsx](src/components/awards/AwardsLayout.tsx); replaced the stub at [app/awards/page.tsx](app/awards/page.tsx) with a Server Component that auth-gates via `auth()` → `redirect("/login")`, composes Header (with `currentPath="/awards"` so `NavLinks` flips `aria-current="page"`), inline Keyvisual + AwardsTitleBlock, AwardsLayout, KudosBlock, Footer, WidgetButton. **Phase 4 (US2):** Client island [AwardsNav.tsx](src/components/awards/AwardsNav.tsx) with `useSyncExternalStore` for hashSlug + `useState<AwardSlug \| null>(scrolledSlug)` overlay; observer uses live `getBoundingClientRect()` per fire and only considers cards with `rect.top ∈ [0, viewportH]` (post-fix from initial entry-snapshot approach which falsely picked top-talent when 28px remained at viewport bottom); hashchange handler scrolls the new target into view so programmatic `replaceState + dispatchEvent` syncs correctly; `prefers-reduced-motion: reduce` swaps smooth scroll for instant. Touch target ≥ 44 × 44 CSS-px (52 × 178 px). Wrapped in `hidden lg:block` per Q-HTG4. **Phase 5 (US3 + US4 verification):** all cross-link + header/footer parity scenarios green via existing components. **Phase 6 (Polish):** edge cases — unknown slug → fallback to `top-talent` with no console error, JS-disabled native anchor scroll works, image alt text non-empty, locale switch preserves DOM identity, 360 px viewport hides menu. **vitest.setup.ts** got jsdom polyfills (IntersectionObserver stub with `globalThis.__triggerIntersection`, scrollIntoView no-op, matchMedia default). Coverage: Vitest 261/261 (38 files), Playwright awards spec 19/19. Lint + tsc clean. **Pre-existing Homepage E2E failures** (auth-redirect scenarios 3 + 5) confirmed unaffected by this work via baseline run on stashed branch. |
