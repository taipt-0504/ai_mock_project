...
| Days unit | `2268:35139` (`1_Days`) | Two-digit zero-padded display + `DAYS` label. **Each digit renders inside its own glassmorphic LED tile** (Figma "Digital Numbers" / 7-segment display, `lg` size: 77×123 per tile, golden border `#FFEA9E`, white→white-10% gradient under `backdrop-filter: blur(24.96px)`) — see Phase 8 visual redesign in `tasks.md`. Range `00`+. Shows `00 00` if `< 1` day remains. | None — display only. Auto-updates per the `Countdown` minute tick. |
| Hours unit | `2268:35144` (`2_Hours`) | Two-digit zero-padded display + `HOURS` label. Same two-LED-tile structure as `1_Days`. Range `00`–`23`. Out-of-range upstream values normalize to `00` (existing `Countdown` arithmetic guarantees this by construction). | None — display only. |
| Minutes unit | `2268:35149` (`3_Minutes`) | Two-digit zero-padded display + `MINUTES` label. Same two-LED-tile structure as `1_Days`. Range `00`–`59`. Out-of-range upstream values normalize to `00` (same guarantee). | None — display only. |
...
- **FR-010a (Demo bypass — exception to FR-003 / US4)** _(added 2026-05-10, Phase 16)_: For this portfolio / demo build the prelaunch surface MUST render exactly ONE additional control — a "Skip pre-launch (demo)" submit button below the countdown — that posts to a Server Action setting an httpOnly cookie `saa_gate_bypass=1` (path `/`, sameSite `lax`, 7-day TTL) and redirects to `/`. While the cookie is present the proxy MUST treat the gate as lifted FOR THAT USER ONLY (other in-flight requests with no cookie keep redirecting); revisiting `/coming-soon` redirects to `/` instead of rendering the prelaunch screen. A small banner mounted in the root layout MUST surface on every post-gate page when the cookie is set, with a "Turn off bypass" button that calls a paired Server Action clearing the cookie and redirecting to `/coming-soon`. This carves a deliberate exception in FR-003 / US4 (no buttons / no `<a>`) — the assertion tightens to "no `<a>`, exactly one `<button>`, and that button is the demo bypass". Removing this control reverts the prelaunch surface to the strict US4 invariant; in a real production deployment of this codebase that revert would be expected before launch.
