...
| `src/components/home/Countdown.tsx`                                    | Client island. Per-minute tick + `visibilitychange` recompute. Renders three units; **each two-digit value is split into a row of two glassmorphic LED tiles** (DSEG7-Classic font via `next/font/local`, golden border, white→white-10% gradient under `backdrop-filter: blur`). `size?: "md" \| "lg"` switches between Homepage (51×82) and Prelaunch (77×123) tile dimensions; see Phase 14 in `tasks.md`. |
