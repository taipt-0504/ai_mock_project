...
|------|--------|----------------------|-------|
| 2026-05-11 | Metadata survey (T005 — Kudos Live Board Phase 1) | `w4WUvsJ9KI` (Profile người khác) + `6-1LRz3vqr` (Tất cả thông báo) + `J3-4YFIpMM` (Open secret box — canonical, has `Spec Created` tag; `m0zV-VstXX` is a no-revision duplicate) + `onDIohs2bS` (View Kudo) | **Four outgoing nav targets from Sun\* Kudos Live Board confirmed via MoMorph MCP `get_frame`** — fulfills plan T005 (Constitution III evidence-based navigation prerequisite). Metadata-only: each screen's existence + canonical screenId recorded; full UX surveys deferred to dedicated `/momorph.screenflow` runs when each target's own spec is queued. Mapping table added to [`.momorph/contexts/SCREENFLOW.md`](contexts/SCREENFLOW.md) under "Open Items" so Phase 4/5/9 implementers can cite a surveyed source for nav citations. **`J3-4YFIpMM` chosen over `m0zV-VstXX`** for Secret Box dialog reference because it has revision `c1504a9eac8a909c2879f625f08dc6e3` + `Spec Created` tag. **`onDIohs2bS` "View Kudo" identified as the Kudos detail target** for `/sun-kudos/[id]` route locked by Q-LB4. **Plan Q-PLAN6 still resolves to MVP minimal layout**; full Figma survey of `onDIohs2bS` deferred unless additional element (comments / replies) discovered later. |
